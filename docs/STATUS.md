...
- **In-place iSCSI PVC growth does not work** (two independent bugs). Controller side: democratic-csi **v1.9.0** reloads SCST via a `simple-file-writer` helper absent on TrueNAS SCALE (upstream #390) → every expand fails `sudo: simple-file-writer: command not found`; fixed by **v1.9.5** (staged, needs staging validation + chart 0.15.1). Node side: online `resize2fs` hits **EPERM on Talos** (`EXT4_IOC_RESIZE_FS`) regardless of driver version. **To grow a PVC, recreate it** (fresh provision skips `resize2fs`). Surfaced by the jellyfin-cache 5→20 Gi bump; full write-up: [incidents/2026-07-02-jellyfin-cache-pvc-expand-simple-file-writer.md](operations/incidents/2026-07-02-jellyfin-cache-pvc-expand-simple-file-writer.md).
