...
- Reference the existing `.github/workflows/build-mopidy.yml` as the canonical pattern (GITHUB_TOKEN auth, immutable date-sha tag: `YYYY-MM-DD-<sha7>`, multi-arch `linux/amd64,linux/arm64` via QEMU + buildx).
