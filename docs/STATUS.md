...
- **Alertmanager state was non-durable — fixed 2026-08-08 ([#1278](https://github.com/gjcourt/homelab/pull/1278)).** `replicas: 1` on `emptyDir`, so every restart (node drain, chart upgrade, OOM) dropped `/alertmanager` — active **silences** and the **notification log** (dedup/inhibition memory) went with it. Now **2 replicas with hard anti-affinity**: Alertmanager's cluster protocol replicates silences and the nflog between peers, so state survives pod restart, node drain, node loss and rolling upgrades. **Deliberately no PVC** — every StorageClass here is democratic-csi against hestia, so a volume would leave Alertmanager unschedulable during a storage incident, i.e. exactly when alerts matter (and Flux would not surface it: Helm does not own the operator-created StatefulSet). Residual gap: state is still lost if *every* replica restarts at once. Closing that needs node-local volumes alongside HA — shelved in [plans/2026-08-08-alertmanager-local-durability.md](plans/2026-08-08-alertmanager-local-durability.md).
