...
- **Alertmanager state was non-durable — fixed 2026-08-08 ([#1278](https://github.com/gjcourt/homelab/pull/1278)).** `alertmanagerSpec.storage` was empty, so the pod ran on `emptyDir`: every restart (node drain, chart upgrade, OOM) dropped `/alertmanager`, taking active **silences** (a silence set during maintenance would evaporate mid-window and the alerts it muted would re-fire) and the **notification log** (dedup/inhibition memory, so already-notified alerts re-notified) with it. Now a 1Gi `truenas-iscsi` PVC via `storage.volumeClaimTemplate`, with `persistentVolumeClaimRetentionPolicy: Retain`. This buys silence + dedup durability **and nothing else** — see the next bullet.
