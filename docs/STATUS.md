...
- **TODO — no historical / fired-alert visibility in Alertmanager.** The AM UI (`alerts.burntbytes.com`) only shows *currently active* alerts — Alertmanager is stateless by design and keeps no alert history, so alerts that fired and resolved leave no trail there. Compounding it: many alerts route to `receiver: 'null'` (Watchdog, `overture-prod` KubePdb, `*-stage` warnings — `infra/controllers/kube-prometheus-stack/values.yaml`) and warnings email to `gjcourt+alerts@` (a Gmail skip-inbox filter, opt-in label), so a firing alert can be real yet invisible in the inbox. The Status page's **`Cluster Status: disabled` is expected, not a bug** — `alertmanagerSpec.replicas: 1`, so there's no HA gossip cluster. **Action:** add a way to review historically-fired alerts — a Grafana alert-history / state-timeline panel off Prometheus `ALERTS` / `ALERTS_FOR_STATE`, or persist AM notifications to a store — and confirm the currently-firing HomeAssistant alert's route (`null` vs `+alerts` vs `+critical`). Fits the [monitoring-enhancement plan](plans/2026-05-09-monitoring-enhancement.md).
