...
- **No historical / fired-alert visibility in Alertmanager.** The AM UI (`alerts.burntbytes.com`) shows only *currently active* alerts — Alertmanager is stateless by design and keeps no history, so alerts that fired and resolved leave no trail there. Two routing facts compound it (`infra/controllers/kube-prometheus-stack/values.yaml`): the **default route receiver is `null`**, so any alert lacking a `severity` label — or otherwise unmatched — is silently dropped (the #1080-class gap); and warnings deliver to `gjcourt+alerts@` behind a Gmail skip-inbox filter (opt-in label), so even a routed warning can go unseen. (Three alerts are *intentionally* muted to `null`: Watchdog, `overture-prod` KubePdb, `*-stage` warnings.) Not a bug: the Status page's `Cluster Status: disabled` is expected for a single-replica AM (`alertmanagerSpec.replicas: 1`, no gossip cluster). **Fix path:** review fired alerts via a Grafana alert-history / state-timeline panel off Prometheus `ALERTS` / `ALERTS_FOR_STATE`, or persist AM notifications to a store — worth a dedicated `docs/plans/` entry when picked up.
